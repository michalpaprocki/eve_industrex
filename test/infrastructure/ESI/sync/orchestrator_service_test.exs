defmodule Infrastructure.ESI.Sync.OrchestratorServiceTest do
  ...
  describe "orchestrate/7" do
    ...
    test "discovers route group on initial sync", %{
      resource_type: resource_type,
      strategy: strategy,
      generation: generation
    } do
      assert {:ok, 1, generation.id} ==
               OrchestratorService.orchestrate(
                 fetch_fn(esi_response(200, [], "1")),
                 generation.id,
    ...
    test "stores an etag for a generation", %{
    ...
    test "changes an etag when the cache is renewed, invalidates current generation and enqueues new one",
         %{resource_type: _resource_type, strategy: strategy, generation: generation} do
      Ecto.Changeset.change(generation, %{
        snapshot_last_modified:
          DateTimeParser.parse_datetime!("Fri, 19 Jun 2026 07:31:36 GMT", to_utc: true)
    ...
    test "1 page resource on successful sync returns :ok, amount of pages and gen id", %{
      resource_type: _resource_type,
      strategy: strategy,
      generation: generation
    } do
      assert {:ok, 1, generation.id} ==
               OrchestratorService.orchestrate(
                 ...
                 5,
                 strategy,
                 metadata(nil, nil, "Fri, 19 Jun 2026 07:31:36 GMT"),
                 1
               )
    ...
    test "multi page resource on successful sync returns :fanout and amount of pages on completion of 1st page",
         %{resource_type: _resource_type, strategy: strategy, generation: generation} do
      assert {:fanout, 20} ==
               OrchestratorService.orchestrate(
                 fetch_fn(esi_response(200, [], "20")),
                 generation.id,
                 strategy.next_generation,
                 1,
                 5,
    ...
    test "multi page resource on successful sync returns :ok on completion of subsequent pages",
         %{resource_type: _resource_type, strategy: strategy, generation: generation} do
      Ecto.Changeset.change(generation, %{
        snapshot_last_modified:
          DateTimeParser.parse_datetime!("Fri, 19 Jun 2026 07:31:36 GMT", to_utc: true)
    ...
    test "after being rate limited on 1st attempt, returns :snooze, delay", %{
      resource_type: _resource_type,
      strategy: strategy,
      generation: generation
    } do
    ...
    test "after rate limited on subsequent attempts, returns :snooze, delay", %{
      resource_type: _resource_type,
      strategy: strategy,
      generation: generation
    } do
    ...
    test "after rate limited and reached max attempts, returns generation as failed", %{
      resource_type: _resource_type,
      strategy: strategy,
      generation: generation
    } do
    ...
    test "returns :ok when etag did not expire, generation is marked as not modified", %{
      resource_type: _resource_type,
      strategy: strategy,
      generation: generation
    } do
    ...
    test "when receiving 500 status codes, emits snooze, delay", %{
      resource_type: _resource_type,
      strategy: strategy,
      generation: generation
    } do
    ...
    test "when receiving 404 status code marks generation as :critical", %{
      resource_type: _resource_type,
      strategy: strategy,
      generation: generation
    } do
    ...
    test "other 400 status codes marks generation as :critical", %{
      resource_type: _resource_type,
      strategy: strategy,
      generation: generation
    } do
    ...
    test "other 300 status codes marks generation as :critical", %{
      resource_type: _resource_type,
      strategy: strategy,
      generation: generation
    } do
    ...
    test "unknown status codes marks generation as :critical", %{
      resource_type: _resource_type,
      strategy: strategy,
      generation: generation
    } do
