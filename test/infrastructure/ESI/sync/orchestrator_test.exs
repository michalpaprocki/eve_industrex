defmodule Infrastructure.ESI.Sync.OrchestratorTest do
  ...
  describe "initiate_paginated_resource_sync/4" do
    ...
    test "creates a new generation for a multi page sync op, stores snapshot etag and expires_at, returns :fanout page number and generation id",
         %{resource_type: _resource_type, strategy: strategy} do
      result =
        Orchestrator.initiate_paginated_resource_sync(
          strategy.id,
          0,
          5,
          fetch_fn(esi_response(200, [], "20", "test_etag"))
        )
  ...
  describe "sync_paginated_resource/6" do
    ...
    test "proceeds with syncing page 2 out of 3, returns :ok, increments pages completed", %{
      ...
      assert :ok ==
               Orchestrator.sync_paginated_resource(
                 strategy.id,
                 generation.id,
                 1,
                 5,
                 fetch_fn(esi_response(200, [], "3")),
                 "2"
               )
  ...
  describe "finalize/3" do
    ...
    test "finalizes a strategy when number of pages completed is equal to pages total", %{
