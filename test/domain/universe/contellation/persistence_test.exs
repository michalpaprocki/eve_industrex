defmodule Domain.Universe.Contellation.PersistenceTest do
  ...
  test "updates a single constellation" do
    assert {:ok, %Constellation{} = constellation} =
             Persistence.upsert(Map.replace(@constellation, :name, "updated_name"))
  ...
  test "updates multiple constellations" do
    ...
    updated =
      Enum.map(@constellations, fn c ->
        %{c | :region_id => c.region_id, :name => "updated_name"}
      end)
