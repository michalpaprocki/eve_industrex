defmodule Domain.Universe.Station.MapperTest do
  ...
  test "Converts esi response into unified maps", context do
    ...
    assert(
      Enum.each(maps, fn m ->
        Map.has_key?(m, :system_id) && Map.has_key?(m, :services) &&
          Map.has_key?(m, :reprocessing_stations_take) &&
          Map.has_key?(m, :reprocessing_efficiency) && Map.has_key?(m, :station_id) &&
          Map.has_key?(m, :name)
      end),
      "Every map in the list has :system_id, :station_id, :reprocessing_stations_take, :reprocessing_efficiency, :services and :name"
    )
