defmodule EveIndustrexWeb.Market.LpShopLive do
  ...
  @sorting_options [
    ...
    %{name: "ISK/LP ▲", type: "isk_lp_asc"},
    %{name: "ISK/LP ▼", type: "isk_lp_desc"}
  ]
  ...
  def mount(params, _session, socket) do
    ...
    params = %{
      "selected_corp" => select_corp(corps, path),
  ...
  def render(assigns) do
    ...
    <div class="text-xl mb-10 h-30 flex flex-col gap-5 mt-10">
      <div class="flex gap-3 flex-col items-center top-20 left-0 w-full">
        <h1 class="font-headers text-3xl">
          {if @form[:selected_corp].value != nil,
            do: Enum.find(@corps, fn c -> c.corp_id == @form[:selected_corp].value end).name} Loyalty Points Shop Browser
        </h1>
        <%= if @form[:selected_corp].value != nil do %>
          <img
            class="w-80 opacity-15 h-80 blur-sm fixed -z-10"
            src={"https://images.evetech.net/corporations/#{@form[:selected_corp].value}/logo?size=256"}
          />
        <% end %>
      </div>
    ...
    <div
      class={"flex w-full bg-black/70 backdrop-blur-sm top-20 left-0 sticky justify-between transition-all xl:justify-center shadow-sm shadow-black delay-0 duration-500 rounded-b-md z-10  #{if @show_form, do: "h-[12rem] lg:h-[8rem]", else: "h-0"}"}
    ...
    >
      ...
      >
        <div class="px-4 py-2 flex items-center gap-2">
          <.input
            field={@form[:selected_corp]}
            label="Corporation Select"
            prompt="Select Corporation..."
            type="select"
            options={Enum.map(@corps, fn c -> [key: c.name, value: c.corp_id] end)}
            id="corp_select"
            class="mt-0 text-base text-ei-text"
          />
        ...
        <div class="flex items-center lg:self-center self-end gap-2 justify-between px-4">
          <.input
            ...
            label="Search Item"
            type="text"
            class="mt-0 min-w-[15ch] text-base"
          />
          <.input
            field={@form[:sorter]}
            label="Sort"
      ...
      <%= cond do %>
        ...
        <% @filtered_offers != nil -> %>
          <%= for o <- sort(@filtered_offers, @form[:sorter].value) do %>
            <.live_component
              id={~s"#{o.offer_id}"}
              module={EveIndustrexWeb.LpShop.LpShopItem}
  ...
  def handle_async(:get_lp_offers, {:ok, result}, socket) do
    %{
      ...
      :offers => _offers,
      :tax_rate => _tax_rate,
      :initial_tax_rate => _initial_tax_rate
    } = socket.assigns

    params = %{
      selected_corp: form[:selected_corp].value,
      selected_trade_hub: form[:selected_trade_hub].value
    }
    ...
    {:noreply,
     socket
     |> assign(:offers, AsyncResult.ok(result))
  ...
  def handle_async(:get_orders, {:ok, result}, socket) do
    ...
    {:noreply,
     ...
     |> assign(:offers, AsyncResult.ok(offers))
     |> assign(:filtered_offers, filtered_offers)}
  end
    ...
    if params["selected_corp"] == "" do
      ...
      cond do
        corp_id != form[:selected_corp].value ->
          path =
            "/market/lp_shop/#{form[:selected_trade_hub].value}/#{corp_id}/#{form[:order_type].value}" <>
        ...
        true ->
          path =
            "/market/lp_shop/#{form[:selected_trade_hub].value}/#{form[:selected_corp].value}/#{form[:order_type].value}" <>
              LiveParser.maybe_compose_query(filter, sorter)
  ...
  def handle_info(
        {:update_price, type, %{target_id: offer_id, type_id: type_id, price: price}},
        socket
      ) do
    ...
    if is_map(filtered_offers) and Map.has_key?(filtered_offers, offer_id) do
      ...
      {:noreply,
       socket
       |> assign(:offers, Map.replace(offers, :result, update))
       |> assign(:filtered_offers, filtered_update)}
    else
      update =
        Map.replace(
          offers.result,
  ...
  defp select_corp(corps, path) do
    if Map.has_key?(path, "corp_id") and
         Enum.any?(corps, fn corp -> corp.corp_id == String.to_integer(path["corp_id"]) end) do
  ...
  defp maybe_start_async(socket, selected_corp),
    do:
