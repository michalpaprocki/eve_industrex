...
alias EveIndustrexWeb.Endpoint
  ...
  def mount(_params, _session, socket) do
    ...
    state_map = Map.new(state, fn {k,v} ->
      {k,v}
    end)
    ready = Enum.all?(state, fn {_k,v} ->
      v == true
    end)
  ...
  def render(assigns) do
    ~H"""
      <div class="flex flex-col">
        <h1 class="text-5xl font-headers text-center my-10">EveIndustr<span class="trailing-text">EX</span></h1>
        <%= if @ready do %>
