defmodule EveIndustrexWeb.Dashboard.DashboardLive do
  ...
  def mount(_params, _session, socket) do
    sync_runtime = :ets.tab2list(:sync_runtime) |> Enum.sort(&(elem(&1, 1).started_at > elem(&2, 1).started_at))
    ...
    strategies_count = Enum.map(resource_types, fn r -> %{name: r.name, count: Query.get_resource_strategies_count(r.name).count} end)
    params = %{"step" => 10}
  ...
  def render(assigns) do
    ~H"""
      <section class="flex flex-col">
        ...
        <span class="self-center text-white p-2 font-semibold text-xl"><%= DateTime.to_date(@time) %> <%= DateTime.to_time(@time) %> </span>
        ...
        <div class="p-2 ring-1 ring-black rounded-md text-white bg-black/70">
          ...
          <.form for={@form} id="step_form" phx-change={"validate_form"}>
            <.input field={@form[:step]} value={@form[:step].value} type={"select"} class="w-fit mb-4 text-black" label={"Refresh rate"} name="step" options={Enum.map(@options, fn x -> x end)} id={"step_select"}/>
          </.form>
          ...
          <div class="flex flex-col gap-1">
            <%= for {k, v} <- @runtime do %>
              <div class="flex flex-col gap-1 py-2">
                ...
                <.live_component module={EveIndustrexWeb.Dashboard.RuntimeDetails} id={k} details={v}/>
              </div>
            <% end %>
          </div>
        <% end %>
