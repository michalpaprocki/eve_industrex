    ...
    <div class="">
      <%= if Enum.any?(@bp_materials, fn bpm -> bpm.order == :missing_order end) do %>
        <div
          ...
          phx-target={@myself}
          class="cursor-pointer hover:text-white hover:bg-black transition p-1 flex flex-col justify-center bg-red-500 animate-pulse text-white"
        >
      ...
      <% else %>
        <div
          phx-click="toggle_modal"
          phx-target={@myself}
          class="cursor-pointer hover:text-white hover:bg-black transition p-1 flex flex-col justify-center"
        >
          <%= if @runs == 1 do %>
            <span class="text-center">
              {Utils.format_with_coma(
                List.foldl(@bp_materials, 0, fn x, acc ->
                  if x == 0, do: 0, else: x.order.price * x.amount + acc
                end)
              ) <> " ISK"}
            </span>
          <% else %>
          ...
          <%= for m <- @bp_materials do %>
            <div class="p-1 flex gap-2 items-center justify-between">
              ...
              <.live_component
                ...
                selected_order={m.order}
                selected_trade_hub={@selected_trade_hub}
                item={%{:type_id => m.type_id, :category_id => m.category_id, :name => m.name}}
                offer_id={@offer_id}
              />
            </div>
          <% end %>
