...
@image_url "https://images.evetech.net/types/"
  ...
  def update(%{:update => %{:type_id => type_id}}, socket) do
    ...
    {:ok, socket |> assign(:item, type) |> assign(:image_url, image_url) |> assign(:average_price, average_price)}
  end
  ...
  def render(assigns) do
    ~H"""
      ...
      <div class="flex text-sm flex-col p-1">
        ...
        <span>Average Price: <%= if @average_price.average_price != nil, do:  Utils.format_with_coma(@average_price.average_price), else: "N/A" %> ISK</span>
      </div>
