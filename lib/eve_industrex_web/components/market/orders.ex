defmodule EveIndustrexWeb.Market.Orders do
  ...
  def update(assigns, socket) do
    sorted =
      if assigns.is_buy_list?,
        do: Enum.sort(assigns.data, &(&1.price > &2.price)),
        else: Enum.sort(assigns.data, &(&1.price < &2.price))
        ...
        <thead>
          <%= if @is_buy_list? do %>
            <tr class="">
              ...
              <th class="border-1 border-solid border-black/20 w-[7%] sticky top-0 bg-ei-bg">
                expires
              </th>
              <th class="border-1 border-solid border-black/20 w-[7%] sticky top-0 bg-ei-bg">
                issued
          ...
          <%= Enum.map(@data, fn o -> %>
            <%= if o.location != nil do %>
              <tr class="px-2 font-sm hover:bg-ei-hover hover:text-white">
                <td class="px-[0.5rem] truncate border-r-[1px] border-stone-800">
                  {o.location.region}
                </td>
                <td class="px-[0.5rem] text-end" border-r-[1px] border-stone-800>
                  {Utils.format_with_coma(o.volume_remain)}
                </td>
                <td class="px-[0.5rem] text-end truncate border-r-[1px] border-stone-800">
                  {Utils.format_with_coma(o.price)} &nbsp;ISK
                </td>
                <td class="px-[0.5rem] text-start truncate border-r-[1px] border-stone-800">
                  <span class={
                ...
                <% end %>
                <td class="px-[0.5rem] text-start border-r-[1px] border-stone-800">
                  {Utils.get_time_left(o.issued, o.duration)}
                </td>
