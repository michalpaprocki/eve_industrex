defmodule EveIndustrexWeb.Market.Filter do
  ...
  def update(assigns, socket) do
    {:ok,
     socket
     |> assign(assigns)
     |> assign(:sec_status_filter, %{"highsec" => "true", "lowsec" => "true", "nullsec" => "true"})
    ...
    <div class="h-[25%] p-1 right-0 truncate">
      <div class="flex items-center gap-1">
        ...
        <input
          id="location_filter"
          value={@search_string}
          class="rounded-md p-1 panel"
          type="text"
          placeholder="🔎 Region / Location..."
          phx-target={@myself}
          phx-keyup="filter_by_location"
          phx-debounce={1000}
        />
      </div>
      <div class="p-1">
        <.input
          ...
          checked={true}
          label="Highsec"
          phx-click="filter_by_sec_status"
          phx-target={@myself}
          phx-value-sec_status="highsec"
        />
        <.input
          type="checkbox"
          name="lowsec"
  ...
  def handle_event("filter_by_location", %{"key" => _key, "value" => value}, socket) do
    send(
      self(),
      {:filter,
       %{:search_string => value, :sec_status_filter => socket.assigns.sec_status_filter}}
    )
  ...
  def handle_event("filter_by_sec_status", %{"sec_status" => sec_status}, socket) do
    ...
    send(
      self(),
      {:filter, %{:sec_status_filter => filter, :search_string => socket.assigns.search_string}}
    )
