defmodule EveIndustrexWeb.Market.Category do
  ...
  def update(assigns, socket) do
    ...
    {:ok,
     socket
     |> assign(assigns)
     |> assign(:cat_indent, cat_indent)
     |> assign(:types, [])
     |> assign(:open, false)
     |> assign(:open_types, false)
     |> assign(:children, Store.get_market_group_children(market_group.market_group_id))
     |> assign(:types, Enum.sort(Store.get_market_group_types(market_group.market_group_id)))}
  end
  ...
  def render(assigns) do
    ...
    <div class="flex flex-col font-headers font-semibold text-base">
      ...
      >
        <.glyph
          name="chevron-right"
          class={" #{if @open or @open_types,  do: "rotate-90", else: "rotate-0"} transition"}
        />
        {@market_group.name}
      </span>
