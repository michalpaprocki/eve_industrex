defmodule EveIndustrexWeb.LpShop.LpBpMaterials do
  ...
  def update(assigns, socket) do
    ...
    {:ok,
     socket |> assign(assigns) |> assign(:show_modal, false) |> assign(:all_present, all_present)}
  end
    ...
    <div class="flex">
      <%= if !@all_present do %>
        <div
          ...
          phx-target={@myself}
          class="cursor-pointer hover:text-ei-accent hover:bg-black transition p-1 flex flex-col justify-center bg-ei-warninig animate-pulse text-white"
        >
      ...
      <% else %>
        <div
          phx-click="toggle_modal"
          phx-target={@myself}
          class="cursor-pointer hover:text-ei-accent hover:bg-black transition p-1 flex flex-col justify-center"
        >
          <%= if @runs == 1 do %>
            <span class="text-center font-bold">
              {Utils.format_with_coma(calc_total(@bp_materials, @bp_material_prices)) <> " ISK"}
            </span>
          <% else %>
            <span class="text-center font-semibold">
              {Utils.format_with_coma(calc_total(@bp_materials, @bp_material_prices)) <>
                " ISK per run"}
            </span>
            <span class="text-center font-bold">
              {Utils.format_with_coma(calc_total(@bp_materials, @bp_material_prices) * @runs) <>
                " ISK total"}
            </span>
          <% end %>
        </div>
          ...
          <%= for m <- @bp_materials do %>
            <div class="p-1 flex gap-2 items-center justify-between text-white">
              ...
              <.live_component
                ...
                amount={m.quantity}
                id={@id<>"_#{m.type_id}_MiniMarket_BP_Materials"}
                ...
                item={
                  %{
                    ...
                    :name => m.name,
                    :price => @bp_material_prices[m.type_id]
                  }
                }
  ...
  defp calc_total(bp_materials, prices) do
    List.foldl(bp_materials, 0, fn x, acc ->
      if x == 0, do: 0, else: Map.get(prices, x.type_id) * x.quantity + acc
    end)
  end
end
