defmodule EveIndustrexWeb.Alchemy.MaterialCost do
  ...
  def update(%{:update => %{:selected_price => selected_price}} = _assigns, socket) do
    ...
    EveIndustrexWeb.Alchemy.Total.update_component(socket.assigns.parent_id <> "_total", %{
      :category => socket.assigns.category,
      :material_id => socket.assigns.material_id,
      :price => price,
  ...
  def update(%{:update => %{:tax_rate => tax_rate}}, socket) do
    if socket.assigns.category == :product && Map.has_key?(socket.assigns, :selected_price) &&
         socket.assigns.selected_price != nil do
      EveIndustrexWeb.Alchemy.Total.update_component(socket.assigns.parent_id <> "_total", %{
        :category => socket.assigns.category,
        :material_id => socket.assigns.material_id,
        :price => socket.assigns.selected_price * ((100 - tax_rate) / 100),
        :amount => socket.assigns.amount
      })
    end
  ...
  def update(assigns, socket) do
    ...
    {:ok,
     socket |> assign(assigns) |> assign(:parent_id, parent_id) |> assign(:material_id, material)}
  end
  ...
  def render(assigns) do
    ...
    <div class="p-1">
      <%= if Map.has_key?(assigns, :selected_price) &&  Map.has_key?(assigns, :tax_rate) && @selected_price != nil do %>
        <span>
          {if @category == :product,
            do: Utils.format_with_coma(@amount * (@selected_price * (100 - @tax_rate) / 100)),
            else: Utils.format_with_coma(@amount * @selected_price)}&nbsp;ISK
        </span>
      <% else %>
