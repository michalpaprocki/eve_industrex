        ...
        <div class="flex flex-col gap-2 p-1">
          ...
          <div class="flex flex-col gap-2 items-between">
            ...
            <.live_component
              ...
              profit_component_id={~s"#{@id}_reaction_profit"}
              id={~s"#{@id}_total"}
            />
            <%= for p <- act.products do %>
              <div class="flex gap-2 items-start justify-between">
                <.live_component
                  module={EveIndustrexWeb.Alchemy.Product}
                  category={assigns.category}
                  id={~s"#{@id}_product"}
                  profit_component_id={~s"#{@id}_reaction_profit"}
                  product={p}
                  hub={@selected_trade_hub}
                  orders={@orders}
                />
              </div>
            <% end %>
            <div>
              <.live_component
                module={EveIndustrexWeb.Alchemy.ReactionProfit}
                id={~s"#{@id}_reaction_profit"}
              />
            </div>
          </div>
        </div>
