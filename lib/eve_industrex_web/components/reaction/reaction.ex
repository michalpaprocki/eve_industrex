    ...
    ~H"""
      <div class="p-2 text-sm rounded-md flex flex-col bg-surface border-1 border-surface-border">
        ...
        <div class="p-1 flex justify-between">
            ...
            <%= if not is_nil(@reaction.profit) do %>
              <span class="text-ei-text"><%= Utils.format_with_coma(@reaction.profit - ((@eiv * @structure_tax) +(@eiv * @ssc_tax) + (@eiv * @system_cost_index |> Market.Service.apply_fw_discount(@fw_upgrade)))) %> ISK</span>
            <% else %>
              <span class="text-ei-text">N/A </span>
            <% end %>
        ...
        <div class="p-1 flex flex-col">
          ...
          <div class="flex p-1 gap-1 items-center">
            ...
            <% else %>
            <span class="text-ei-text-muted"><%= Utils.format_with_coma(Enum.reduce(@reaction.bp.activities.reaction.materials, 0, fn m, acc -> @reaction.prices.materials[m.type_id] * m.quantity + acc end)) %> ISK</span>
            <% end %>
        ...
        <div class="p-1 flex justify-between">
          ...
          <span class="text-ei-text-muted"><%= Utils.format_with_coma(@eiv) %> ISK
          </span>
        </div>
        <div class="p-1 flex justify-between">
          <span title="Job installation Fee" class="text-ei-text-muted">Job Fee: </span>
          <span class="text-ei-text-muted"><%= Utils.format_with_coma(@eiv * @system_cost_index |> Market.Service.apply_fw_discount(@fw_upgrade)) %> ISK
          </span>
        </div>
          ...
          <div class="p-1">
            ...
            <div class="p-1 flex justify-between">
              ...
              <span class="text-ei-text-muted"><%= Utils.format_with_coma(@eiv * @ssc_tax) %> ISK
              </span>
            </div>
            <div class="p-1 flex justify-between">
              <span title="Structure Tax" class="text-ei-text-muted">Structure Tax: </span>
              <span class="text-ei-text-muted"><%= Utils.format_with_coma(@eiv * @structure_tax) %> ISK
              </span>
            </div>
            <div class="p-1 flex justify-between">
              <span title="Total Taxes" class="text-ei-text-muted">Total: </span>
              <span class="text-ei-text-muted"><%= Utils.format_with_coma((@eiv * @structure_tax) +(@eiv * @ssc_tax) + (@eiv * @system_cost_index |> Market.Service.apply_fw_discount(@fw_upgrade))) %> ISK
              </span>
            </div>
