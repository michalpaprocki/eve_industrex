defmodule EveIndustrexWeb.CoreComponents do
  ...
  use Gettext, backend: EveIndustrexWeb.Gettext
        ...
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              ...
              class="shadow-zinc-700/10 ring-zinc-700/10 relative hidden rounded-2xl bg-black/70 p-14 shadow-lg ring-1 transition"
            >
  ...
  def flash(assigns) do
    ...
    <div
      ...
      role="alert"
      class={[
        "font-headers fixed top-2 right-2 mr-2 w-80 sm:w-96 z-50 rounded-lg p-3 ring-1",
        @kind == :info && "bg-surface text-ei-text ring-ei-accent fill-cyan-900",
        @kind == :error && "bg-surface text-ei-critical shadow-md ring-ei-critical fill-rose-900"
      ]}
      {@rest}
    >
      ...
      <button type="button" class="group absolute top-1 right-1 p-2" aria-label={gettext("close")}>
        <.glyph name="x" class="h-5 w-5 opacity-40 group-hover:opacity-70" />
      </button>
    </div>
    """
  end
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(EveIndustrexWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(EveIndustrexWeb.Gettext, "errors", msg, opts)
    end
  end
