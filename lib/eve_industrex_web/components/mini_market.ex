  ...
  def handle_event("select_order", %{"price" => price, "order_id" => order_id} = _params, socket) do
    ...
    case socket.assigns.category do
      :product ->
        ...
        send(self(), {:update_price, :product, %{target_id: socket.assigns.target_id, price: String.to_float(price), type_id: socket.assigns.item.type_id}})
      :req_item ->
        send(self(), {:update_price, :req_item, %{target_id: socket.assigns.target_id, price: String.to_float(price), type_id: socket.assigns.item.type_id}})
      :bp_materials ->
        send(self(), {:update_price, :bp_materials, %{target_id: socket.assigns.target_id, price: String.to_float(price), type_id: socket.assigns.item.type_id}})
      _->
        :noop
    end
    ...
    if length(form.errors) == 0 do
      ...
      case socket.assigns.category do
        :product ->
          ...
          send(self(), {:update_price, :product, %{target_id: socket.assigns.target_id, price: form[:custom_price].value, type_id: socket.assigns.item.type_id}})
        :req_item ->
          send(self(), {:update_price, :req_item, %{target_id: socket.assigns.target_id, price: form[:custom_price].value, type_id: socket.assigns.item.type_id}})
        :bp_materials ->
          send(self(), {:update_price, :bp_materials, %{target_id: socket.assigns.target_id, price: form[:custom_price].value, type_id: socket.assigns.item.type_id}})
        _->
          :noop
      end
      {:noreply, socket |> assign(:selected_order, selected_order) |> assign(:show_modal, false)}
