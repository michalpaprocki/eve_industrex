defmodule EveIndustrexWeb.MiniMarket do
  ...
  def update(assigns, socket) do
    params = %{
      "custom_price" => 0.00,
      "order_type" => get_order_type(assigns.order_type, assigns.category)
    }
    ...
    {:ok,
     socket
     |> assign(:orders_fetched, false)
     |> assign(:is_loading?, false)
     |> assign(:form, to_form(changeset, as: :order_form))
     |> assign(assigns)
     |> assign(:show_modal, false)
     |> assign(:selected_order, selected_order)
     |> assign(:types, @types)
     |> assign(:selected_type, hd(@types))
     |> assign(:updated_tax_rate, nil)}
  end
      ...
      <%= cond do %>
        <% @selected_order.price == nil -> %>
          <span
            ...
            class="cursor-pointer hover:text-ei-hover hover:bg-black transition p-1 bg-ei-critical animate-pulse text-white rounded-sm"
          >
        ...
        <% true -> %>
          <span
            phx-target={@myself}
            phx-click="toggle_modal"
            class="cursor-pointer hover:text-ei-hover hover:bg-black rounded-sm transition p-1 font-semibold"
          >
                ...
                <%= case @item.category_id do %>
                  ...
                  <% _ -> %>
                    <img
                      ...
                      src={"https://images.evetech.net/types/#{@item.type_id}/icon?size=128"}
                    />
                <% end %>
            ...
            <div class={"#{if @is_loading? == true, do: "hidden"} flex flex-col gap-2 py-4"}>
              <.form for={@form} phx-target={@myself} phx-change="validate">
                ...
                <.input
                  ...
                  type="select"
                  label="Order Type"
                  options={Enum.map(@types, fn t -> t end)}
                />
              ...
              <tbody class="">
                <%= if @is_loading? == false do %>
                  <%= Enum.map(get_order_on_type(@orders, @order_type, @category), fn o -> %>
                    <tr
                      class={"px-2 font-sm #{if @selected_order.id == o.order_id, do: "bg-surface text-ei-accent", else: "hover:bg-black hover:text-ei-hover cursor-pointer"}"}
                      phx-target={@myself}
                      phx-click="select_order"
                      phx-value-price={o.price}
                      phx-value-order_id={o.order_id}
                    >
                      <td class="pl-2 text-end">
                        {Utils.format_with_coma(o.volume_remain)} / {Utils.format_with_coma(
                          o.volume_total
                        )}
                      </td>
                      <td class="pl-2 text-end truncate">
                        {Utils.format_with_coma(o.price)} &nbsp;ISK
                      </td>
                      <td class="pl-2 text-start truncate">
                        <span class={
                          apply_color_on_status(
                            :erlang.float_to_binary(o.location.security_status, decimals: 1)
                          )
                        }><%= :erlang.float_to_binary(o.location.security_status, [decimals: 1]) %></span>&nbsp;{o.location.name}
                      </td>
                    </tr>
                  <% end) %>
                <% end %>
  ...
  def handle_event(
        "validate",
        %{"_target" => ["custom price"], "custom price" => price, "order type" => _order_type} =
          _params,
        socket
      ) do
  ...
  def handle_event(
        "validate",
        %{"_target" => ["order type"], "custom price" => _price, "order type" => order_type} =
          _params,
        socket
      ) do
    %{:form => form} = socket.assigns
        ...
        send(
          ...
          {:update_price, :product,
           %{
             ...
             type_id: socket.assigns.item.type_id
           }}
        )
  ...
  def handle_async(:get_market_orders, {:ok, result}, socket) do
    {:noreply,
     socket
     |> assign(:orders_fetched, true)
     |> assign(:orders, result)
     |> assign(:is_loading?, false)}
  end
