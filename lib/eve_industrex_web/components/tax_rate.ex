defmodule EveIndustrexWeb.Common.TaxRate do
      ...
      >
        <div class="flex gap-2 items-start md:flex-row flex-col">
          <.input
            class=""
            field={@form[:tax_rate]}
            value={@form[:tax_rate].value}
            label="Sales Tax Range:"
            type="range"
            min={0}
            max={7.5}
            step={0.01}
            maxlength={3}
            name="sales tax range"
            id="sales_tax_range"
            phx-debounce={1000}
          />
          <.input
  ...
  def handle_event(
        "validate_form",
        %{"_target" => ["sales tax range"], "sales tax range" => tax} = _params,
        socket
      ) do
  ...
  def handle_event(
        "validate_form",
        %{"_target" => ["sales tax input"], "sales tax input" => tax} = _params,
        socket
      ) do
    changeset = validate_input(tax, socket.assigns.form)
