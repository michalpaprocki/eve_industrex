defmodule EveIndustrex.Industry.BlueprintActivity.Mapper do
  ...
  def from_dump(
        {"manufacturing", %{"materials" => mats, "products" => prods, "skills" => skills} = map}
      ) do
    %{
      ...
      materials:
        Enum.map(mats, fn m ->
          EveIndustrex.Industry.BlueprintActivityMaterial.Mapper.from_dump(m)
        end),
      products:
        Enum.map(prods, fn p ->
          EveIndustrex.Industry.BlueprintActivityProduct.Mapper.from_dump(p)
        end),
      skills:
        Enum.map(skills, fn s ->
          EveIndustrex.Industry.BlueprintActivitySkill.Mapper.from_dump(s)
        end)
    }
  end
  ...
  def from_dump(
        {"invention", %{"products" => prods, "materials" => mats, "skills" => skills} = map}
      ) do
    %{
  ...
  def from_dump({"reaction", %{} = map}) do
    %{
      ...
      materials:
        Enum.map(Map.get(map, "materials"), fn m ->
          EveIndustrex.Industry.BlueprintActivityMaterial.Mapper.from_dump(m)
        end),
      products:
        Enum.map(Map.get(map, "products"), fn p ->
          EveIndustrex.Industry.BlueprintActivityProduct.Mapper.from_dump(p)
        end),
      skills:
        Enum.map(Map.get(map, "skills"), fn s ->
          EveIndustrex.Industry.BlueprintActivitySkill.Mapper.from_dump(s)
        end),
      time: Map.get(map, "time")
    }
