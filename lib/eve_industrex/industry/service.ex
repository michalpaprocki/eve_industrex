defmodule EveIndustrex.Industry.Service do
  ...
  def get_reactions_view() do
    ...
    Map.new(formulas, fn {id, f} -> {id, %{type: f, bp: Industry.Blueprint.Store.get_blueprint(id) |> handle_prepare()}} end)
  end

  def extract_reactions_type_ids(reactions) do
    Enum.map(reactions, fn {_id, r} -> extract_bp_type_ids(r.bp) end) |> List.flatten() |> Enum.uniq()
  end
