defmodule EveIndustrex.Infrastructure.ESI.Client do
  ...
  require Logger
  alias EveIndustrex.Infrastructure.ESI.Endpoints.{AveragePrices, MarketOrders, Station, MarketGroup}
  ...
  defp fetch(url) do
    ...
    case Req.get(url) do
      {:ok, %Req.Response{status: status, headers: headers, body: body, private: _private, trailers: _trailers} = _response} ->
        ...
        {:ok, %Response{status: status, body: body, route: url, headers: %Headers{global_error_limit_remain: get_header(headers, "x-esi-error-limit-remain"), global_error_limit_reset: get_header(headers, "x-esi-error-limit-reset") ,last_modified: get_header(headers, "last-modified"), retry_after: get_header(headers, "retry-after") ,pages: get_header(headers, "x-pages"), etag: get_header(headers, "etag"), expires_at: get_header(headers, "expires"), rate_limit: get_header(headers, "x-ratelimit-limit"), rate_limit_used: get_header(headers, "x-ratelimit-used"), rate_limit_remaining: get_header(headers, "x-ratelimit-remaining"), rate_limit_group: get_header(headers, "x-ratelimit-group")}}}
