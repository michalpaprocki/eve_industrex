defmodule EveIndustrex.Infrastructure.ESI.Sync.Query do
  ...
  def get_strategies_count(list_of_resource_type_ids) do
    query =
      from(r in EsiSyncStrategy,
        where: r.resource_type_id in ^list_of_resource_type_ids,
        select: {r.resource_type_id, count(r.id)},
        group_by: r.resource_type_id
      )
  ...
  def get_resource_strategies_count(name) do
    from(r in ResourceType,
      where: r.name == ^name,
      select: %{id: r.id, count: r.strategies_count}
    )
    |> Repo.one()
  end

  def aggregate_strats_count(resource_id) do
    from(s in EsiSyncStrategy, where: s.resource_type_id == ^resource_id)
  ...
  def get_strategies_by_resource(resource_name) do
    from(s in EsiSyncStrategy,
      join: r in ResourceType,
      on: s.resource_type_id == r.id,
  ...
  def claim_due_strategies() do
    ...
    Repo.transaction(fn ->
      strategies =
        from(s in EsiSyncStrategy,
          where: s.enabled == true and s.status in [:failed, :idle] and s.next_run_at <= ^now,
      ...
      from(s in EsiSyncStrategy, where: s.id in ^ids)
  ...
  def get_failed_and_critical_strategies() do
    from(strat in EsiSyncStrategy,
      where: strat.status == :critical and strat.status == :failed and strat.enabled == true,
      select: %{id: strat.id}
    )
    |> Repo.all()
  end
  ...
  def get_total_execution_time() do
    from(gen in EsiSyncGeneration, order_by: [gen.updated_at], limit: 1, select: gen.duration_ms)
    |> Repo.all()
  end
  ...
  def get_missing_rate_limit_groups() do
    from(s in EsiSyncStrategy,
      left_join: g in assoc(s, :rate_limit_group),
      where: is_nil(g.id),
  ...
  def get_current_gen(resource_name) do
    ...
    from(r in ResourceType,
      where: r.name == ^resource_name,
      join: s in EsiSyncStrategy,
      on: r.id == s.resource_type_id,
      join: g in subquery(gen_query),
      on: s.id == g.esi_sync_strategy_id,
  ...
  def strategies_not_running?(resource_type_id) do
    query =
      from(s in EsiSyncStrategy,
        where: s.resource_type_id == ^resource_type_id,
        select: sum(fragment("CASE WHEN ? = ? THEN 1 ELSE 0 END", s.status, "running")) == 0
      )
