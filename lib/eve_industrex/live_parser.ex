defmodule EveIndustrex.LiveParser do
  ...
  def apply_expression(collection, key ,expression) do
    case expression do
      {:profit} ->
         Map.new(
             Enum.filter(collection, fn {_id, o} -> is_number(o[key]) and o[key] > 0 end)
            )
      {:gt, amount} ->
        Map.new(
              Enum.filter(collection, fn {_id, o} -> is_number(o[key]) and o[key] > amount end)
            )
      {:lt, amount} ->
        Map.new(
            Enum.filter(collection, fn {_id, o} -> is_number(o[key]) and o[key] < amount end)
      ...
      {:range, min, max} ->
          Map.new(
            Enum.filter(collection, fn {_id, o} -> is_number(o[key]) and min <= o[key] and o[key] <= max end)
            )
      _ ->
        collection
    end
  ...
  def apply_text_filter(collection, text_filter) do
    case text_filter do
      nil ->
        collection
      _ ->
        group_filter = Enum.filter(collection, fn {_id, o} -> String.contains?(String.downcase(o.type.group) , String.downcase(text_filter)) end)

        if length(group_filter) == 0 do
          Map.new(
            Enum.filter(collection, fn {_id, o} -> String.contains?(String.downcase(o.type.name) , String.downcase(text_filter)) end)
            )
        else
            Map.new(
          group_filter
