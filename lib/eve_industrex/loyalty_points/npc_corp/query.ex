defmodule EveIndustrex.LoyaltyPoints.NpcCorp.Query do
  ...
  def corps_with_offers(),
    do: Store.get_all() |> Enum.map(&elem(&1, 1)) |> Enum.sort(&(&1.name < &2.name))

  def get_corps_with_offers(),
    do:
      from(c in NpcCorp,
        join: o in assoc(c, :offers),
        distinct: true,
        order_by: [asc: c.name],
        select: {c.corp_id, %{corp_id: c.corp_id, name: c.name}}
      )
      |> Repo.all()
