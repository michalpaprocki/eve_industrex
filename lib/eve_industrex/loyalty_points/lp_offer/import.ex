defmodule EveIndustrex.LoyaltyPoints.LpOffer.Import do
  ...
  def from_esi() do
    ...
    Mapper.map_corp_and_offers_ids(fetched_offers)
    |> Enum.chunk_every(100)
    |> Enum.each(fn chunk ->
      EveIndustrex.LoyaltyPoints.CorpOffer.Persistence.upsert_all(chunk)
    end)

    type_ids =
      EveIndustrex.LoyaltyPoints.LpReqItem.Mapper.get_offer_type_ids(offers)
      |> Enum.uniq()
      |> Enum.sort(:asc)
    ...
    missing_type_ids =
      for {t, i} <- Enum.with_index(type_ids) do
    ...
    req_items =
      EveIndustrex.LoyaltyPoints.LpReqItem.Mapper.get_req_items(offers) |> List.flatten()
