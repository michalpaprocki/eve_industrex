defmodule EveIndustrex.Market.MarketOrder.Service do
  ...
  def project_orders_to_cache(tid) do
    ...
    query =
      from(mo in MarketOrder,
        where: mo.generation == subquery(max_gen),
        select: %{
          type_id: mo.type_id,
  ...
  def project_bid_ask_for_trade_hub(tid) do
    ...
    query =
      from(mo in MarketOrder,
        where: mo.generation == subquery(max_gen) and mo.location_id in ^@trade_hub_station_ids,
        group_by: [mo.location_id, mo.type_id],
        select: %{
          location_id: mo.location_id,
          type_id: mo.type_id,
          max_buy: filter(max(mo.price), mo.is_buy_order == true),
          min_sell: filter(min(mo.price), mo.is_buy_order == false)
        }
      )
  ...
  defp to_ets_row(order) do
    {
      {order.type_id, get_order_type(order.is_buy_order)},
      project_order(order)
    }
  end
  ...
  defp to_ets_bid_ask_row(order) do
    {
      {order.location_id, order.type_id},
      {order.max_buy, order.min_sell}
    }
  end
  ...
  defp project_order(order) do
    {order.order_id, order.price, order.volume_remain, order.volume_total, order.min_volume,
     order.location_id, order.range, order.region_id, order.duration, order.issued,
     get_security_status(order.location_id)}
  end
