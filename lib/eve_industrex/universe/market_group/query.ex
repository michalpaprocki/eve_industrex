defmodule EveIndustrex.Universe.MarketGroup.Query do
  ...
  def get_market_groups_for_cache(),
    do:
      from(mg in MarketGroup,
        where: is_nil(mg.parent_group_id),
        order_by: [asc: mg.name],
  ...
  def get_market_groups_children_for_cache(),
    do:
      from(mg in MarketGroup, where: not is_nil(mg.parent_group_id))
      |> Repo.all()
      |> Enum.map(fn mg ->
        {mg.parent_group_id,
         %{
  ...
  def get_market_groups_types() do
    from(mg in MarketGroup,
      where: not is_nil(mg.parent_group_id),
      join: t in Type,
      on: mg.market_group_id == t.market_group_id and t.published == true,
      select: {mg.market_group_id, %{type_id: t.type_id, name: t.name}}
    )
    |> Repo.all()
  end
  ...
  def get_market_groups_with_parents(),
    do: from(mg in MarketGroup, where: not is_nil(mg.parent_group_id)) |> Repo.all()

  def get_market_types_by_query(query) do
    Store.get_types()
    |> Enum.filter(fn {_market_group, type} ->
      String.contains?(String.downcase(type.name), query)
    end)
    |> Enum.map(fn {_market_group, type} -> type end)
  end
  ...
  def return_missing_market_groups(list_of_mgroup_ids) do
    query =
      from(
        v in fragment(
          "SELECT * FROM unnest(?::integer[]) AS market_group_id",
          ^list_of_mgroup_ids
        ),
        left_join: mg in MarketGroup,
        on: mg.market_group_id == v.market_group_id,
