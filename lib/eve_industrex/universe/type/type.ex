defmodule EveIndustrex.Universe.Type do
  ...
  schema "types" do
    ...
    belongs_to :market_group, MarketGroup,
      references: :market_group_id,
      define_field: false,
      foreign_key: :market_group_id

    has_many :reprocess_materials, ReprocessMaterial,
      foreign_key: :source_type_id,
      references: :type_id

    has_many :lp_offers, LpOffer, foreign_key: :offer_id
    # optional blueprint products
    many_to_many :bp_products, __MODULE__,
      join_through: "bp_product_types",
      join_keys: [bp_product: :type_id, type: :type_id],
      on_delete: :delete_all,
      on_replace: :delete

    has_many :average_prices, AveragePrice,
      references: :type_id,
      foreign_key: :type_id,
      on_delete: :delete_all
