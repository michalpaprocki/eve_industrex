defmodule EveIndustrex.Logger.EiLogger do
  ...
  defp write_to_file(type, path, {status, {reason, {module, func, args}}, source}) when is_binary(source) do

    msg = Atom.to_string(type)<>": "<>Atom.to_string(status)<>": reason: [#{Atom.to_string(reason)} | #{Atom.to_string(module)} | #{Atom.to_string(func)} | [#{args |> Enum.map(&to_string/1) |> Enum.join(", ")}]] | "<> "#{(source)}"
    case type do
      :error ->
        Logger.error(msg)
        :noop
